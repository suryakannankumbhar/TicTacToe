...
def drawLines():
    ...
    pygame.draw.line(
        screen, lineColor, (0, 2 * squareSize), (width, 2 * squareSize), lineWidth
    )
    pygame.draw.line(
        screen, lineColor, (squareSize, 0), (squareSize, height), lineWidth
    )
    pygame.draw.line(
        screen, lineColor, (2 * squareSize, 0), (2 * squareSize, height), lineWidth
    )
        ...
        for col in range(boardCols):
            if board[row][col] == 1:
                pygame.draw.line(
                    ...
                    crossColor,
                    (
                        col * squareSize + crossSpace,
                        row * squareSize + squareSize - crossSpace,
                    ),
                    (
                        col * squareSize + squareSize - crossSpace,
                        row * squareSize + crossSpace,
                    ),
                    crossWidht,
                )
                pygame.draw.line(
                    screen,
                    crossColor,
                    (col * squareSize + crossSpace, row * squareSize + crossSpace),
                    (
                        col * squareSize + squareSize - crossSpace,
                        row * squareSize + squareSize - crossSpace,
                    ),
